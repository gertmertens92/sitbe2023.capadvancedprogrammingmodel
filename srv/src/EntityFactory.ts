...
export default class EntityFactory {
    ...
    public static getInstance(): EntityFactory {
        if (!EntityFactory.instance) EntityFactory.instance = new EntityFactory();
    ...
    public getHandlerInstanceService(Service: string, Entity: string): Promise<BaseHandler | undefined> {
        //@ts-ignore
        return this.handlers[Service] && this.handlers[Service][`${Entity}Handler`] ? new this.handlers[Service][`${Entity}Handler`]() : undefined;
    }
    ...
    private async applyServiceEntityHandlers(srv: Service) {
        for (const entity of Object.keys(srv.entities)) {
            const handlerInstanceService = await this.getHandlerInstanceService(srv.name, entity);
            ...
            for (const supportedOperation of handlerInstanceService?.getSupportedOperations()) {
                ...
                (srv[supportedOperation.event] as (eve: Events, entity: Target, handler: OnEventHandler | EventHandler | ResultsHandler) => Service)
                    (supportedOperation.type, entity,
                        afterOperations.includes(supportedOperationFn) ?
                            async (each: any, req: Request) => (await this.getHandlerInstanceService(srv.name, entity))?.[`${supportedOperationFn}Base`]?.(each, req) :
                            async (req: Request, next: any) => (await this.getHandlerInstanceService(srv.name, entity))?.[`${supportedOperationFn}Base`]?.(req, next));
            }
        }
    }
